...
void drawBresenCenterLine(int x_st, int y_st, int x_to, int y_to, int color) // bresenham_line
{
...
int main()
{
    ...
    int ukuran_square = 300; // ukuran 300 dalam pixel
    int sisi_kiri = x_mid - ukuran_square / 2;
    int sisi_atas = y_mid - ukuran_square / 2;
    int sisi_kanan = x_mid + ukuran_square / 2;
    int sisi_bawah = y_mid + ukuran_square / 2;
    drawSquareWarnaUngu(sisi_kiri, sisi_atas, sisi_kanan, sisi_bawah, YELLOW);
    boundaryFill(x_mid, y_mid, MAGENTA, YELLOW);
    
    int panjang_sisi_triangle = 259;
    int titik_atas_triangle = y_mid - 112; 
    int jarak_tengah_y = 37;
    drawTriangleWarnaUnguMuda(x_mid, titik_atas_triangle, 90, panjang_sisi_triangle, WHITE); 
    boundaryFill(x_mid, y_mid + jarak_tengah_y, LIGHTBLUE, WHITE);
    
    int jarijari_circle = 75;
    int pusat_y = y_mid + jarak_tengah_y;
    drawCircleWarnaMerah(x_mid, pusat_y, jarijari_circle, 0, 360, YELLOW);
    boundaryFill(x_mid, pusat_y, RED, YELLOW);
    
    int jarijari_elipse_1 = 75;
    int jarijari_elipse_2 = 20;
    drawMidpointElipseWarnaPink(x_mid, pusat_y, jarijari_elipse_1, jarijari_elipse_2, WHITE);
    boundaryFill(x_mid, pusat_y, LIGHTMAGENTA, WHITE);
    
    drawBresenCenterLine(x_mid, pusat_y - jarijari_elipse_2, x_mid, pusat_y + jarijari_elipse_2, BLACK);
